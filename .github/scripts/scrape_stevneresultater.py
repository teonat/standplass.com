#!/usr/bin/env python3
"""Scrape pistolfelt/spesialfelt competition results from NSF API and write data/felt/{year}.json."""
...
def build_mappings(branchlist_data):
    ...
    # Build discipline_names and find relevant group IDs
    discipline_names = {}
    group_ids = []

    feltpistol_group_id = None
    spesialfelt_group_id = None
    ...
    for grp in pistol_branch.get('disciplineGroups', []):
        ...
        # Identify feltpistol and spesialfelt groups
        if 'felt' in grp_name_lower and 'spesial' not in grp_name_lower:
            if feltpistol_group_id is None:
                feltpistol_group_id = grp['id']
                print(f'Found feltpistol group: "{grp_name}" ({grp["id"]})', flush=True)
            else:
                print(f'WARNING: multiple "felt" groups found; using first. Ignoring "{grp_name}"', file=sys.stderr)
        elif 'spesial' in grp_name_lower:
            if spesialfelt_group_id is None:
                spesialfelt_group_id = grp['id']
                print(f'Found spesialfelt group: "{grp_name}" ({grp["id"]})', flush=True)
            else:
                print(f'WARNING: multiple "spesial" groups found; using first. Ignoring "{grp_name}"', file=sys.stderr)

    if not feltpistol_group_id:
        print('WARNING: No feltpistol disciplineGroup found under Pistol branch', file=sys.stderr)
    if not spesialfelt_group_id:
        print('WARNING: No spesialfelt disciplineGroup found under Pistol branch', file=sys.stderr)

    for gid in [feltpistol_group_id, spesialfelt_group_id]:
        if gid:
            group_ids.append(gid)

    if not group_ids:
        print('ERROR: No relevant discipline groups found — cannot continue', file=sys.stderr)
        sys.exit(1)
